...
1/np.linalg.norm(pks,axis=1).max()
...
for i in range(len(gs)):
    ...
    im = axs[1,i].imshow(np.where(True, np.sqrt(gs[i]['w'][0]**2 + gs[i]['w'][1]**2), np.nan)[dr:-dr,dr:-dr].T, interpolation='none')
...
axs[0].set_title(f'sigma={sigma}, kstep={kstep}')
...
axs[1].set_title(f'sigma={sigma}, kstep={kstep}')
...
reconstructed3 = ndi.map_coordinates(np.nan_to_num(np.where(smask, GPAim, np.nan)), [xxh2+unew_iso[0], yyh2+unew_iso[1]], cval=np.nan) 
...
#ax[0].imshow(reconstructed.T, vmax=np.nanquantile(reconstructed, 0.999), vmin=np.nanquantile(reconstructed, 0.001),
             origin='lower',
           cmap='gray')
...
plt.plot(slices[2][:10].mean(axis=0))
X = slices[2][:10].mean(axis=0)
X = np.abs(X-X.mean()) > 200
plt.scatter(np.arange(slices[2][:10].mean(axis=0).shape[0])[X], slices[2][:10].mean(axis=0)[X], c='C1')

plt.plot(slices[2][:10].mean(axis=0)[binary_erosion(~X, [True]*5)])
plt.plot(slices[2][:10].mean(axis=0)[~X])

for newEslice in slices:
    # Use MM to crop out edge outliers
    X = newEslice[:10].mean(axis=0, keepdims=True)
    #X = np.abs(X-X.mean()) > 100
    newEslice = newEslice / X
    plt.figure()
    plt.imshow(np.log(newEslice / newEslice[:,230:236].mean(axis=1, keepdims=True))[24:].T, 
               aspect='auto', extent=[EGY[24], EGY[-1],-1,1], 
               cmap='seismic', vmax=0.3,vmin=-0.3)
    plt.yticks([-1,-1/3,0,1/3,1], ['AA','AB','SP','BA','AA']);
    plt.colorbar()
...
for newEslice in slices:
    ...
    #newEslice = newEslice / X
    plt.figure()
    plt.imshow((newEslice / newEslice.mean(axis=1, keepdims=True))[24:].T, 
               aspect='auto', extent=[EGY[24], EGY[-1],-1,1], 
               #cmap='seismic', vmax=0.3,vmin=-0.3
              )
    plt.yticks([-1,-1/3,0,1/3,1], ['AA','AB','SP','BA','AA']);
...
for newEslice in slices:
    ...
    #X = np.abs(X-X.mean()) > 100
    #newEslice = newEslice / X
    newEslice = np.abs(newEslice-1)
    plt.figure()
    plt.imshow(newEslice.T, 
               aspect='auto', extent=[EGY[24], EGY[-1],-1,1], 
               #cmap='seismic', vmax=0.3,vmin=-0.3
              )
    plt.yticks([-1,-1/3,0,1/3,1], ['AA','AB','SP','BA','AA']);
...
for ind, l in zip([-8.,-4.,8.], ['AB', 'SP', 'AA']):
    axs[0].semilogy(Krasovenergy, ds.Reflectivity.sel(dx=ind).data.T, label=l, color=stackingcolors[l])
    axs[1].semilogy((hibino[l]['Energy']-11), hibino[l]['(0,0)']*2, label=l, color=stackingcolors[l])
...
for i,label in enumerate(['AA', 'AB', 'BA', 'AA']):
    ax.annotate(label, center + i/3*(avecs[2]+avecs[1]), 
                        ...
                        #color=stackingcolors[label],
                        color='white',
                       bbox=dict(boxstyle="square,pad=0.2", lw=0, fc=stackingcolors[label], alpha=0.5))
for vec in [avecs[1], avecs[2], avecs[2]+avecs[1]]:
    ax.annotate('SP', center + 0.5*(vec), 
                            fontweight='bold', ha="center", va="center",
                            #color=stackingcolors[label],
                            color='white',
                           bbox=dict(boxstyle="square,pad=0.2", lw=0, fc=stackingcolors['SP'], alpha=0.5))
...
plt.plot(np.diff(np.quantile(ldat, [0.01,0.99], axis=1), axis=0).T)
