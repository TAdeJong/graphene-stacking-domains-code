...
# wavelengths has units of pks, i.e. periods per pixel (?)
...
np.any(np.array(rgbmasks), axis=0).shape, (normmag > cutoff).shape
...
ax[0].imshow(smooth[xslice, yslice].T, cmap='gray',
             vmax=np.quantile(cropped, 0.999),
             #alpha=0.5,
             vmin=0.75)
...
ax[1].imshow(smooth[xslice, yslice].T, cmap='gray',
             vmax=np.quantile(cropped, 0.9999), 
             #alpha=0.5, 
             vmin=0.75)
...
plt.savefig(os.path.join('plots', 'BSstrainhist2.pdf'), dpi=600)
# -
