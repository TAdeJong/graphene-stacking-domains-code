...
f, ax = plt.subplots(figsize=[12,12])
im = fftplot(ndi.filters.gaussian_filter(fftim, sigma=5), pcolormesh=False, ax=ax, vmax=2e3)
fftr=0.2
plt.xlim(-fftr,fftr)
plt.ylim(-fftr,fftr)
plt.colorbar(im, ax=ax)

pk , _ = GPA.extract_primary_ks(clipped - clipped.mean(), plot=True, sigma=15, pix_norm_range=(70,200), DoG=False)
...
(~np.isnan(np.stack([np.where(normmag > 1.5, 0.246 / (2*NMPERPIXEL/wavelengths[i]) * 100, np.nan).ravel() for i in range(3)]))).sum(axis=1)
